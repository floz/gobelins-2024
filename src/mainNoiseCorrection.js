...
import CircleNoise from "./elements/CircleNoiseCorrection"
...
pane.addBinding( data, 'noisePosRatio', { min: 0, max: .04, step: .001 } ).on( 'change', draw )
pane.addBinding( data, 'noisePosStr', { min: 0, max: 50, step: .1 } ).on( 'change', draw )
pane.addBinding( data, 'color', { view: 'color' } ).on( 'change', initAndDraw )
pane.addBinding( data, 'alpha', { min: 0, max: 1, step: .01 } ).on( 'change', draw )
pane.addBinding( data, 'activateBlendMode' ).on( 'change', draw )
...
function init() {
	// get hsl color from data.color
	const hsl = hexToHSL( data.color, { toString: false } )
	hsl.h *= 360
	...
	for( let ix = 0; ix < data.cols; ix++ ) {
		for( let iy = 0; iy < data.lines; iy++ ) {
			...
			circles.push( new CircleNoise( x, y, noise, Object.assign( {}, hsl ) ) )
		}
	}
}
